...
你的任务：将其转化为单个 JSON 场景，遵循剧本格式。

规则：
1. 场景标题格式：内景/外景 地点 - 时间
2. 动作描述行：现在时，展示而非讲述，只描写可见/可闻的内容
3. 对白：忠实于每个角色的语气。括号提示（parenthetical）少用
4. characters_present 必须使用 context.characters[].id；dialogue.character 也必须使用同一个角色 id，不要使用角色名
5. 绝不包含 forbidden_info 中的信息——它们是未来的剧透
6. 保留所有 key_plot_points
7. 可以省略 omit_details 中列出的细节
8. 内心想法必须转化为可见的动作或对白
JSON 场景结构：
{
  "id": "scene_NNN",
  "sequence": N,
  "heading": "内景/外景 地点 - 时间",
  "setting": { "location": "...", "time": "...", "interior": true/false },
  "synopsis": "一句话场景概要",
  "characters_present": ["char_id_1", "char_id_2"],
  "content": [
    { "type": "action", "text": "..." },
    { "type": "dialogue", "character": "char_id", "parenthetical": "(可选)", "text": "..." },
    { "type": "transition", "text": "切至：" }
  ]
}
只输出该单个场景的 JSON，不要 markdown 代码块，不要额外注释。`
...
func (a *SceneWriterAgent) WriteScene(ctx context.Context, sceneCtx *scene.SceneContext) (*schema.Scene, error) {
	...
	prompt := fmt.Sprintf("Generate the scene for this context package:\n```json\n%s\n```", string(ctxJSON))
	raw, err := Run(ctx, a.agent, prompt)
	if err != nil {
		return nil, err
	}
	raw, err = util.PrepareJSON(raw, "scene writer output")
	if err != nil {
		return nil, err
	}
	var sc schema.Scene
	if err := json.Unmarshal([]byte(raw), &sc); err != nil {
		return nil, fmt.Errorf("failed to parse scene JSON: %w\nraw: %s", err, raw)
	}
	return &sc, nil
}
