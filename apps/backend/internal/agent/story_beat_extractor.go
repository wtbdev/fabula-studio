...
type StoryBeatExtractorAgent struct {
	...
	customExtractWindow func(context.Context, segment.BeatWindow) ([]segment.StoryBeat, error)
}
...
func (a *StoryBeatExtractorAgent) Extract(ctx context.Context, idx *segment.SourceIndex) ([]segment.StoryBeat, error) {
	return a.ExtractWithConcurrency(ctx, idx, 1)
}

// ExtractWithConcurrency extracts and repairs story beats for the complete source index.
// Windows are processed concurrently, then reconciled in source order so overlap repair
// and final beat IDs remain deterministic.
func (a *StoryBeatExtractorAgent) ExtractWithConcurrency(ctx context.Context, idx *segment.SourceIndex, concurrency int) ([]segment.StoryBeat, error) {
	...
	results := make([][]segment.StoryBeat, len(windows))
	jobs := make(chan int)
...
func (a *StoryBeatExtractorAgent) extractWindow(ctx context.Context, window segment.BeatWindow) ([]segment.StoryBeat, error) {
	if a.customExtractWindow != nil {
		return a.customExtractWindow(ctx, window)
	}
	payload := struct {
