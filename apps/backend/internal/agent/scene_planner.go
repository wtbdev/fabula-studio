...
你将收到两类输入之一：
1. 旧版叶节点列表（已分析完毕、可进行改编的故事片段）。
2. 新版 SceneCandidate 列表（由已校正 StoryBeat 构建，带稳定 source_sentence_ids）。

对每个节点或候选节拍，决定它如何映射到场景：
...
- id: "plan_NNN"
- source_node_ids: 涉及的输入 ID 数组；新版输入必须使用 candidate ID，不要输出 beat ID 或 sentence ID
...
func (a *ScenePlannerAgent) PlanScenes(ctx context.Context, leaves []*tree.StoryNode) ([]*scene.ScenePlan, error) {
	...
	return scene.ValidateAndRepairScenePlans(plans, nil), nil
}
